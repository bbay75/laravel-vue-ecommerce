<template>
 <header class="flex justify-between items-center p-4 h-14 shadow bg-white">
        <button @click="emit('toggle-sidebar')" class="flex items-center justify-center rounded transition-colors w-8 h-8  text-gray-700 hover:bg-black/10">
          <Bars3BottomLeftIcon class="w-6"/>
        </button>
          <Menu as="div" class="relative inline-block text-left">
        <MenuButton class="flex items-center">
          <img src="https://randomuser.me/api/portraits/men/33.jpg" class="rounded-full w-8 mr-2">
...
<script setup>
  import {  Bars3BottomLeftIcon, ArrowRightOnRectangleIcon, UserCircleIcon } from '@heroicons/vue/24/outline'
